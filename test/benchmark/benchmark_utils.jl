...
function aggregate_trials(aggregated_trials, test, chain)
    chain = [chain; test]
    if haskey(SampleProjectTest.benchmark_results, test)
        trial = minimum(SampleProjectTest.benchmark_results[test])
...
function save_benchmark(test_results, runtime_seconds, file_name)
    ...
    dict["config"] = @sprintf("seconds=%.1f", SampleProjectTest.benchmark_duration_seconds)
